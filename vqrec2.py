...
def differentiable_topk(x, k, temperature=1.):
    batch_size, sequence_length, num_codes, code_dim = x.shape
    topk_tensors = []
    ...
    return topks.reshape(batch_size, sequence_length, k * num_codes, code_dim)
...
class VQRec(SequentialRecommender):
    ...
    def code_projection(self, seq):
        embedded_seq = self.trm_encoder(seq)
        codebook = self._get_codebook()
        assigned_idx = (codebook.unsqueeze(0) - embedded_seq.unsqueeze(1)).norm(dim=-1).argmin(dim=1)
        reassigned_code_embedding = self.codebook_embeddings(assigned_idx)
        return reassigned_code_embedding

    #def code_projection(self):
        doubly_stochastic_matrix = gumbel_sinkhorn(torch.exp(self.trans_matrix), n_iters=self.sinkhorn_iter)
        trans = torch.ceil(differentiable_topk(doubly_stochastic_matrix.unsqueeze(0), 1).squeeze(2))
        trans = trans[..., 1:].cumsum(dim=-1)
        
        embedded_seq = self.trm_encoder(self.seq)
        embedded_seq = self.dropout(embedded_seq)
        encoded_seq = self.quantize(embedded_seq)
    # assign code to each token in the sequence
        assigned_idx = encoded_seq.argmax(dim=-1)
    # reassign codes based on learned probabilities
        if self.reassigned_code_embedding is not None:
        # Use the learned codebook for code projection
          assigned_idx = self.reassigned_code_embedding.argmax(dim=-1)
    # map assigned codes to codebook vectors
        codebook_vectors = self.codebook(assigned_idx)
    # concatenate embedded sequence with codebook vectors
        projected_seq = torch.cat([embedded_seq, codebook_vectors], dim=-1)
        return projected_seq
    # quantize embedded sequence to codebook
        if self.index_assignment_flag:
          assigned_idx = self.reassigned_code_embedding.argmax(dim=-1)
        else:
          assigned_idx = self.pq_codes.argmax(dim=-1)
          self.codebook_index = trans.gather(dim=-1, index=assigned_idx[..., None])[..., 0]
          self.codebook_index = self.codebook_index.long()
          self.codebook_index += (torch.arange(self.codebook_index.size(0), device=self.codebook_index.device) * self.code_dim)[:, None]
          self.codebook_index = self.codebook_index.view(-1)
          self.reassigned_code_embedding = self.pq_code_embedding.weight.index_select(0, self.codebook_index).view(
           -1, self.max_seq_length, self.code_dim
         ) 
        return self.reassigned_code_embedding
    ...
    def forward(self, item_seq, item_seq_len, **kwargs):
        ...
        trm_output = self.trm_encoder(input_emb, extended_attention_mask, output_all_encoded_layers=True,  **kwargs)
    ...
    def calculate_loss(self, interaction, **kwargs):
        ...
        item_seq = interaction[self.ITEM_SEQ]
        item_seq_len = interaction[self.ITEM_SEQ_LEN]
        seq_output = self.forward(item_seq, item_seq_len, **kwargs)
