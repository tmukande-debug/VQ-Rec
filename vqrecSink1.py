...
class VQRec(SequentialRecommender):
    def __init__(self, config, dataset):
        ...
        self.to_token_emb = nn.Embedding(self.num_tokens, self.emb_dim)
        self.position_embedding = nn.Embedding(self.max_seq_len, self.hidden_size)
        #self.item_emb = nn.Embedding(num_embeddings=7680, embedding_dim=300)
    ...
    def _get_outputs(self, interaction):
    # Get the sequence of items
      seq = interaction[:, :-1]

    # Embed the sequence of items
      seq_emb = self.item_emb(seq)

    # Get the last item in the sequence
      last_item = interaction[:, -1]

    # Embed the last item
      last_item_emb = self.item_emb(last_item)

    # Return the sequence embedding and the last item embedding
      return seq_emb, last_item_emb
      
    def calculate_loss(self, interaction):
      seq_output, test_item_emb = self._get_outputs(interaction)
      logits = torch.matmul(seq_output, test_item_emb.transpose(0, 1))

    # Calculate dot product instead of matrix multiplication
      logits = torch.sum(seq_output[:, :, None] * test_item_emb[None, :, :], dim=1)

    # Compute loss
      targets = interaction["item_id"].unsqueeze(1).repeat(1, logits.shape[1])
