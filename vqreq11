...
#import torch
def lehmer_encode(rankings):
    device = rankings.device
    batch_size, n_codes = rankings.size()
    code = torch.zeros((batch_size, n_codes), device=device, dtype=torch.float32)
    for i in range(n_codes):
        code[:, i] = torch.sum(rankings[:, i:] < rankings[:, i, None], dim=-1)
    return code
...
def lehmer_sinkhorn(A, n_iters=10):
    r = torch.ones_like(A.sum(1))
    c = torch.ones_like(A.sum(0))
    for i in range(n_iters):
        r /= torch.matmul(A, c.unsqueeze(-1)).squeeze(-1)
        c = 1 / torch.matmul(A.transpose(-1, -2), r.unsqueeze(-1)).squeeze(-1)
        r /= torch.matmul(A, c.unsqueeze(-1)).squeeze(-1)
    return torch.diag(r) @ A @ torch.diag(c)
